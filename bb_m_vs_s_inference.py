...
def run_experiment():
    ...
    for test_can in [1, 5, 10]:
        ...
        if service_api:
            output_file = output_file + '_service_api'
        if not os.path.exists(output_file + f'_bb_m_vs_s_{d_type}.p'):
            pickle.dump(ssd_stat_dict, open(output_file + f'_bb_m_vs_s_{d_type}.p', 'wb'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--service_api', action='store_true', default=False)
    ...
    attack_verbose = True#False
    query_verbose = True#False
    metric_verbose = True
